...
//
//CUDA_HOSTDEV
//	tTrkLocPos UtilityMethods::SimpleRtTrackGlobal2Local(tTrackSeg* segmentArray, int trackSegIterator, int nTrackSegs, tdble X, tdble Y, int type)
//{
//
//	tTrkLocPos p;
//	int 	segnotfound = 1;
...
//
//CUDA_HOSTDEV
//	double UtilityMethods::normPI_PI(double angle){
//
...
//
//CUDA_HOSTDEV
//	double UtilityMethods::mod(double num){
//	return num > 0 ? num : -1 * num;
//}
//
//CUDA_HOSTDEV
//	double UtilityMethods::norm0_2PI(double num) {
//
//	double PI = 3.14159265358979323846;
//
//	while ((num) > 2 * PI) {
...
//
//CUDA_HOSTDEV
//bool ConstraintChecking::validPoint(tTrackSeg* segArray, int nTrackSegs, State* targetState, double distFromSides){
...
//
//CUDA_HOSTDEV
//double EvalFunctions::evaluatePathCost(tTrackSeg* segmentArray, int nTrackSegs, State* s1, State* s2, int fwdLimit){
//
//	tTrkLocPos l1, l2;
//
...
//
//CUDA_HOSTDEV
//double EvalFunctions::oldEvaluatePathCost(State* s1, State* s2, int fwdLimit){
//
//	tTrkLocPos l1, l2;
//
...
//
//CUDA_HOSTDEV
//double EvalFunctions::evaluateStateCost(State* s1, State* s2, double actionSimDeltaTime){
...
//CUDA_HOSTDEV
//void DeltaHeuristics::lineHeuristic(int neighboorDeltaPos, State* state, State* parent, double diffPathCost){ //only accounts the position
//
...
//CUDA_HOSTDEV
//void DeltaHeuristics::quadraticBezierHeuristic(double neighboorDeltaPos, double neighboorDeltaSpeed, State* state, State* parent, double diffPathCost){
//
//	float PI = 3.14159265358979323846f;
//
...
////accounts for the position and speed (quadratic)
//CUDA_HOSTDEV
//void DeltaHeuristics::smoothBezierHeuristic(double neighboorDeltaPos, double neighboorDeltaSpeed, State* state, State* parent, double diffPathCost){
//
//	float PI = 3.14159265358979323846f;
//
...
//CUDA_HOSTDEV
//void DeltaHeuristics::cubicBezierHeuristic(double neighboorDeltaPos, double neighboorDeltaSpeed, State* state, State* parent, double diffPathCost){ //accounts for the position and speed
//
//	float PI = 3.14159265358979323846f;
//
//	float curvePercent = 0.5;
...
////wrapper
//CUDA_HOSTDEV
//void DeltaHeuristics::applyDelta(State* state, State* parent, tTrackSeg* trackSegArray, int nTrackSegs, int forwardSegments, double neighborDeltaPos, double neighborDeltaSpeed){
//
